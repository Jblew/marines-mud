 ...
 * @author jblew
 */
public class MyObjectSpace extends ObjectSpace {
    static private final Object instancesLock = new Object();
    static MyObjectSpace[] instances = new MyObjectSpace[0];
    static private final HashMap<Class, MyObjectSpace.CachedMethod[]> methodCache = new HashMap();
    ...
    private final Listener invokeListener = new Listener() {
        public void received(Connection connection, Object object) {
            if (!(object instanceof MyObjectSpace.InvokeMethod)) {
                return;
            }
            if (connections != null) {
                ...
                if (i == n) {
                    return; // The MyObjectSpace.InvokeMethod message is not for a connection in this ObjectSpace.
                }
            }
            MyObjectSpace.InvokeMethod invokeMethod = (MyObjectSpace.InvokeMethod) object;
    ...
    public MyObjectSpace() {
        synchronized (instancesLock) {
            MyObjectSpace[] instances = MyObjectSpace.instances;
            MyObjectSpace[] newInstances = new MyObjectSpace[instances.length + 1];
            newInstances[0] = this;
            System.arraycopy(instances, 0, newInstances, 1, instances.length);
            MyObjectSpace.instances = newInstances;
        }
    }

    /** Creates an ObjectSpace with the specified connection. More connections can be {@link #addConnection(Connection) added}. */
    public MyObjectSpace(Connection connection) {
        this();
    ...
    public void unregister(int objectID) {
        ...
        if (TRACE) {
            trace("kryonet", "Object unregistered in ObjectSpace id=" + objectID);
        }
    }
    ...
    public void close() {
        ...
        synchronized (instancesLock) {
            ...
            temp.remove(this);
            instances = temp.toArray(new MyObjectSpace[temp.size()]);
        }
    ...
    protected void invoke(Connection connection, Object target, MyObjectSpace.InvokeMethod invokeMethod) {
        if (DEBUG) {
            String argString = "";
        ...
        MyObjectSpace.InvokeMethodResult invokeMethodResult = new MyObjectSpace.InvokeMethodResult();
    ...
    static public RemoteObject getRemoteObject(Connection connection, int objectID, Class... ifaces) {
        ...
        return (RemoteObject) Proxy.newProxyInstance(ObjectSpace.class.getClassLoader(), temp, new MyObjectSpace.RemoteInvocationHandler(
                connection, objectID));
    }
    ...
    static private class RemoteInvocationHandler implements InvocationHandler {
        ...
        final ArrayList<MyObjectSpace.InvokeMethodResult> responseQueue = new ArrayList();
        ...
        public RemoteInvocationHandler(Connection connection, final int objectID) {
            ...
            responseListener = new Listener() {
                public void received(Connection connection, Object object) {
                    if (!(object instanceof MyObjectSpace.InvokeMethodResult)) {
                        return;
                    }
                    MyObjectSpace.InvokeMethodResult invokeMethodResult = (MyObjectSpace.InvokeMethodResult) object;
        ...
        public Object invoke(Object proxy, Method method, Object[] args) {
            ...
            MyObjectSpace.InvokeMethod invokeMethod = new MyObjectSpace.InvokeMethod();
            ...
            synchronized (responseQueue) {
                while (true) {
                    ...
                    for (int i = responseQueue.size() - 1; i >= 0; i--) {
                        MyObjectSpace.InvokeMethodResult invokeMethodResult = responseQueue.get(i);
    ...
    static MyObjectSpace.CachedMethod[] getMethods(Kryo kryo, Class type) {
        MyObjectSpace.CachedMethod[] cachedMethods = methodCache.get(type);
        ...
        int n = methods.size();
        cachedMethods = new MyObjectSpace.CachedMethod[n];
        for (int i = 0; i < n; i++) {
            MyObjectSpace.CachedMethod cachedMethod = new MyObjectSpace.CachedMethod();
    ...
    static Object getRegisteredObject(Connection connection, int objectID) {
        ...
        for (int i = 0, n = instances.length; i < n; i++) {
            MyObjectSpace objectSpace = instances[i];
    ...
    static public void registerClasses(Kryo kryo) {
        ...
        kryo.register(MyObjectSpace.InvokeMethod.class);

        FieldSerializer serializer = (FieldSerializer) kryo.register(MyObjectSpace.InvokeMethodResult.class).getSerializer();
